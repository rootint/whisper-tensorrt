    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cuda_compute_capability = list(
            nvmlDeviceGetCudaComputeCapability(nvmlDeviceGetHandleByIndex(0))
        )
        nvmlShutdown()
...
def build_trt_engine(model_name="large-v2", args=None, force=False, log_level="error"):
    ...
    with RunningStatus(
        ...
        out_logs = (
            os.popen(
                f"python3 -m whisper_s2t.backends.tensorrt.engine_builder.builder --output_dir='{args.output_dir}' --log_level='{log_level}'"
            )
            .read()
            .split("\n")
        )
